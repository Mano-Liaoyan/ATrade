...
| AppHost/template transport defaults now use HTTPS for iBeam: the `ibkr-gateway` resource is declared with an HTTPS endpoint, and `.env.template` defaults `ATRADE_IBKR_GATEWAY_URL` to `https://127.0.0.1:5000`. | Implemented in Step 1; shell contract tests will be updated in Step 2 for the new expected scheme. | `src/ATrade.AppHost/Program.cs`, `.env.template` |
