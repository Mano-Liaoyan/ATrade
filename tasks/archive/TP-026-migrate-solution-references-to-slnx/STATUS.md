...
| The committed environment-template fallback is required by active startup docs/tests. | Kept `.env.template` as the safe committed template; no real credentials or account identifiers added. | `.env.template`, `scripts/local-env.sh` |
...
| 2026-04-30 | Step 1 root detection migrated | `LocalDevelopmentPortContract` now recognizes `ATrade.slnx` before the retained legacy `.sln` compatibility file and `.env.template`. |
...
| 2026-04-30 | Step 1 targeted script triage | First targeted script sweep passed `project-shells` and `provider-abstraction`, then failed `market-data-feature` before solution use because the committed environment-template fallback was missing; restored `.env.template` and will rerun modified scripts. |
...
| 2026-04-30 | Step 4 failures fixed | Earlier targeted-script failure from missing `.env.template` was fixed by restoring the safe template; the regression-guard self-audit message was corrected; all Step 4 commands now pass. |
