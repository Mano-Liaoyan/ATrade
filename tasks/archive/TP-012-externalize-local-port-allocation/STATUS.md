...
| Repo-level `.env` contract does not exist yet; `find .env*` returned no files and `.gitignore` already ignores developer-local `.env`. | Add a committed template (likely `.env.template`) plus fallback defaults instead of committing a machine-specific `.env`. | `.gitignore`, repository root |
...
| Chosen contract shape: committed repo-root `.env.template` plus ignored developer-local `.env`. | Keep portable defaults committed while allowing per-machine overrides in `.env`. | `.env.template`, `.gitignore` |
| Defined contract variables: `ATRADE_API_HTTP_PORT`, `ATRADE_FRONTEND_DIRECT_HTTP_PORT`, and `ATRADE_APPHOST_FRONTEND_HTTP_PORT`. | Use explicit names tied to the direct API path, direct frontend path, and AppHost-managed frontend path. | `.env.template` |
| Intentionally ephemeral/internal ports remain outside the contract. | `.env.template` now explicitly leaves AppHost `127.0.0.1:0` internals and service target ports (`5432`/`6379`/`4222`) in code instead of freezing them in `.env`. | `.env.template`, `src/ATrade.AppHost/Properties/launchSettings.json`, `src/ATrade.AppHost/Program.cs` |
| Fallback behavior is defined at the contract boundary. | `.env.template` documents that local startup and test helpers should use the committed template values whenever a developer-local `.env` file is absent. | `.env.template` |
...
| Startup path and contract resolution are now centralized. | The shared loader resolves repo root + frontend path in C#, while `scripts/local-env.sh` lets `./start run` load the same `.env`/`.env.template` contract without duplicating path logic; wrapper verification honored `ATRADE_APPHOST_FRONTEND_HTTP_PORT=3012`. | `src/ATrade.ServiceDefaults/LocalDevelopmentPortContract.cs`, `scripts/local-env.sh`, `scripts/start.run.sh` |
...
| Stale duplicated port literals were removed from verification paths. | The tests now derive API/frontend manifest/runtime expectations from `ATRADE_*` values instead of repeating `5181`/`3111`/`3000` in multiple places; only the committed `.env.template` template keeps the canonical defaults. | `tests/apphost/api-bootstrap-tests.sh`, `tests/apphost/frontend-nextjs-bootstrap-tests.sh`, `tests/apphost/apphost-infrastructure-manifest-tests.sh`, `tests/start-contract/start-wrapper-tests.sh`, `.env.template` |
| CI remains deterministic without a developer-local `.env`. | With `ATRADE_*` overrides unset, the updated shell tests pass by loading committed defaults from `.env.template`; the frontend runtime check now derives its DCP session folder from the current AppHost log instead of stale global process state. | `.env.template`, `tests/apphost/frontend-nextjs-bootstrap-tests.sh`, `tests/apphost/api-bootstrap-tests.sh`, `tests/apphost/apphost-infrastructure-manifest-tests.sh`, `tests/start-contract/start-wrapper-tests.sh` |
...
| Startup docs now describe the repo-level local-port contract. | `scripts/README.md` now documents `.env.template` vs `.env`, the three `ATRADE_*` variables, the intentional exclusions, and the new override verification harness. | `scripts/README.md` |
...
| The repo still boots with default local-port values. | `timeout 20s ./start run` succeeded with the `ATRADE_*` overrides unset, confirming the committed defaults remain runnable without a developer-local `.env`. | `./start run`, `.env.template` |
| The repo-level env contract is now the single source of truth for developer-controlled local ports. | `LocalDevelopmentPortContractLoader` now requires `ATRADE_*` values to come from the environment or `.env`/`.env.template`, the shell helper reads the same contract, and final grep verification showed the only remaining canonical default values live in `.env.template` (plus a test assertion that checks that template). | `.env.template`, `scripts/local-env.sh`, `src/ATrade.ServiceDefaults/LocalDevelopmentPortContract.cs`, `tests/start-contract/start-wrapper-tests.sh` |
...
| 2026-04-24 14:48 | Env template shape chosen | Added repo-root `.env.template` to define the contract while keeping `.env` developer-local |
| 2026-04-24 14:49 | Env variables defined | Added explicit developer-controlled port variable names to `.env.template` |
| 2026-04-24 14:50 | Ephemeral ports preserved | Documented the ports intentionally excluded from the repo-level env contract |
| 2026-04-24 14:50 | Fallback contract documented | Recorded that `.env.template` supplies the default local-port values when `.env` is absent |
...
| 2026-04-24 15:18 | Deterministic test defaults verified | Updated shell tests passed with the contract overrides unset, proving `.env.template` is sufficient for CI defaults |
