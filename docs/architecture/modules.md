...
> market-data provider. `ATrade.Analysis` now defines the provider-neutral
> analysis engine seam, API-facing registry, normalized request/result shapes,
> engine/source metadata, and no-configured-engine fallback that future LEAN or
> alternate engines must implement. `ATrade.Workspaces` owns the first backend-persisted workspace preference: Postgres-backed pinned
...
- **Responsibilities:** In the current slice, provide an ASP.NET Core host
  ...
  provider. The analysis endpoints resolve `IAnalysisEngineRegistry`, expose
  provider-neutral engine discovery/run payloads, and return explicit
  `analysis-engine-not-configured` responses until a concrete engine is
  registered. The watchlist endpoints resolve the temporary local workspace identity, initialize
