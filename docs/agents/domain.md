...
ATrade uses a single-context layout for engineering skills.

- `tasks/CONTEXT.md` is the current repo domain context and durable Taskplane memory.
...
When output names a domain concept, use the terms already present in `tasks/CONTEXT.md` and active architecture docs. Prefer project language such as Exact Instrument Identity, paper-capital source, saved backtest run, Compose-managed infrastructure, AppHost, and provider-neutral contracts over new synonyms.
