...
export default async function BacktestSymbolPage({ params, searchParams }: TerminalSymbolPageProps) {
  const [{ symbol }, resolvedSearchParams] = await Promise.all([
    params,
    searchParams ?? Promise.resolve({}),
  ]);

  return <TerminalRoutePage moduleId="BACKTEST" searchParams={resolvedSearchParams} symbol={symbol} />;
}
