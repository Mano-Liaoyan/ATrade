...
assert_exact_identity_actions() {
  ...
  assert_file_contains "$workflow" "createBacktestNavigationIntent"
  ...
  assert_file_contains "$monitor" "onOpenIntent(workflow.openBacktestIntent(row), buildOpenStatusMessage('BACKTEST', row))"
  assert_file_contains "$table" "href={row.chartHref}"
  assert_file_contains "$table" "href={row.analysisHref}"
  assert_file_contains "$table" "href={row.backtestHref}"
