...
assert_terminal_workflows_reachable() {
  ...
  assert_file_contains "$frontend_root/lib/terminalChartWorkspaceWorkflow.ts" 'SignalR unavailable: HTTP fallback active.'
...
assert_disabled_future_modules_visible_and_honest() {
  ...
  assert_file_contains "$frontend_root/components/terminal/TerminalDisabledModule.tsx" 'Disabled module. No data or order controls are shown.'
  assert_file_contains "$frontend_root/components/terminal/TerminalDisabledModule.tsx" 'No data or order controls are shown.'
  assert_file_contains "$frontend_root/lib/terminalModuleRegistry.ts" 'News provider not configured.'
  assert_file_contains "$frontend_root/lib/terminalModuleRegistry.ts" 'Portfolio workspace not available.'
  assert_file_contains "$frontend_root/lib/terminalModuleRegistry.ts" 'Screener builder not available.'
  assert_file_contains "$frontend_root/lib/terminalModuleRegistry.ts" 'AI assistant not configured.'
  assert_file_contains "$frontend_root/lib/terminalModuleRegistry.ts" 'No order tickets or submit actions.'
