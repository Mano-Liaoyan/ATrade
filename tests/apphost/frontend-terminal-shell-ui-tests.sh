...
assert_terminal_shell_source_contract() {
  ...
  assert_file_not_contains "$app" 'Module-driven paper workspace'
  ...
  assert_file_contains "$repo_root/frontend/components/terminal/MarketMonitorDetailPanel.tsx" 'exact provider identity'
  assert_file_contains "$repo_root/frontend/components/terminal/TerminalHomeModule.tsx" 'Orders are disabled by the paper-only safety contract.'
  assert_file_contains "$app" 'function handleModuleSelect(moduleId: EnabledTerminalModuleId)'
  assert_file_not_contains "$app" '<TerminalCommandInput'
  assert_file_not_contains "$app" 'handleCommand'
  assert_file_not_contains "$app" 'commandFeedback'

  assert_file_absent "$repo_root/frontend/components/terminal/TerminalCommandInput.tsx"
  assert_file_absent "$repo_root/frontend/lib/terminalCommandRegistry.ts"
  ...
  assert_file_absent "$repo_root/frontend/components/terminal/TerminalStatusStrip.tsx"
  assert_file_absent "$repo_root/frontend/lib/terminalLayoutPersistence.ts"
  assert_file_contains "$help_module" 'data-testid="terminal-help-module"'
  assert_file_contains "$help_module" 'Module and workflow reference'
  ...
  assert_file_not_contains "$css" '.terminal-command-input'
  ...
  if grep -RIn --exclude-dir=.next --exclude-dir=node_modules -E 'Bloomberg|BLOOMBERG|BLP|bbg-terminal|bloomberg-terminal' "$repo_root/frontend"; then
    printf 'frontend workspace shell must not include Bloomberg/proprietary terminal assets or trademarks.\n' >&2
    return 1
  fi
}
...
PY
  ...
  assert_file_not_contains "$root_response" 'data-testid="terminal-command-input"'
  assert_file_contains "$root_response" 'data-testid="terminal-module-rail"'
  assert_file_not_contains "$root_response" 'data-testid="terminal-safety-strip"'
  ...
  assert_file_contains "$root_response" 'data-testid="terminal-home-status-grid"'
  assert_file_contains "$root_response" 'data-testid="terminal-home-quick-actions"'
  assert_file_not_contains "$root_response" 'data-testid="terminal-status-strip"'
  ...
  assert_file_not_contains "$root_response" 'Module-driven paper workspace'
  assert_file_contains "$root_response" 'Paper trading command overview'
  ...
  assert_file_contains "$root_response" 'Paper only · no orders'
  ...
  assert_file_not_contains "$chart_response" 'data-testid="terminal-status-strip"'
  ...
  assert_file_contains "$chart_response" 'candles'
